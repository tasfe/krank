 ...
 * are a result of trying to get transaction awareness to work, which it does not right now.
 * 
 * Basically annotated transactions seem to be broken unless using the jpa template.
 * 
 */
...
public class GenericDaoJpaWithoutJpaTemplate<T, PK extends Serializable> implements GenericDao<T, PK>, Finder<T> {
    ...
    public List<T> find( Class clazz, List<Criterion> criteria, List<String> orderBy ) {
        return find( clazz, orderBy.toArray( new String[orderBy.size()] ), (Criterion[]) criteria
    ...
    public List<T> find( Class clazz, List<Criterion> criteria, String[] orderBy ) {
        return find( clazz, orderBy, (Criterion[]) criteria.toArray( new Criterion[criteria.size()] ) );
    }
    ...
    public int count() {
        ...
        Query query = getEntityManager().createQuery( "SELECT count(*) FROM " + entityName + " instance" );
    ...
    public List<T> find( Class clazz ) {
        ...
        Query query = getEntityManager().createQuery( "SELECT instance FROM " + entityName + " instance" );
    ...
    private List<T> doFind( Class clazz, OrderBy[] orderBy, final Criterion[] criteria, Fetch[] fetches,
        ...
        final String sQuery = sbQuery.append( constructSelect( getEntityName( clazz ), "o" ) ).append(
                constructJoins( fetches ) ).append( constuctWhereClause( group ) ).append( constructOrderBy( orderBy ) )
                .toString();
    ...
    private List<T> doFind( Class clazz, String[] orderBy, final Criterion[] criteria, Fetch[] fetches,
        ...
        if (orderBy != null) {
            ...
            return doFind( clazz, list.toArray( new OrderBy[orderBy.length] ), criteria, fetches, startPosition,
                    maxResult );
    ...
    private String constructJoins( Fetch[] fetches ) {
        ...
        for (Fetch fetch : fetches) {
            ...
            builder.append( " join fetch " ).append( fetch.isAliasedRelationship() ? "" : "o." ).append(
    ...
    private String constructSelect( String entityName, String instanceName ) {
        StringBuilder query = new StringBuilder( "SELECT " + ( this.distinct ? "DISTINCT " : "" ) + instanceName
                + " FROM " );
