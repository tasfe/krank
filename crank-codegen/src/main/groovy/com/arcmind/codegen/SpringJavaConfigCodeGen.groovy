...
public class SpringJavaConfigCodeGen{
	...
	private static String ${relationship.owner.name}_${relationship.name}_RELATIONSHIP = "${relationship.name}";'''
...
'''
	...
	public JsfSelectManyController<${relationship.relatedClass.name}, Long> ${relationship.owner.name.unCap()}To${relationship.name}Controller()
		...
		JsfSelectManyController<${relationship.relatedClass.name}, Long> controller = new JsfSelectManyController<${relationship.relatedClass.name}, Long>(
				${relationship.relatedClass.name}.class, ${relationship.owner.name}_${relationship.name}_RELATIONSHIP, paginators().get(${relationship.relatedClass.name.toUpperCase()}), ${relationship.owner.name.unCap()}Crud()
						.getController());
		return controller;
	}
'''
	
	public String processTemplate(ChangeSpec changeSpec, String template, String relationships="", String indent="") {
		StringBuilder builder = new StringBuilder()
		builder << indent + "    /* ${changeSpec.startLocationMarker} */"
		classes.each{JavaClass cls -> builder << engine.createTemplate(template).make([bean:cls, relationships:relationships]).toString()  }
	...
	public String getCrudControllers() {
		processTemplate crudChangeSpec, crudTemplateText
	}
