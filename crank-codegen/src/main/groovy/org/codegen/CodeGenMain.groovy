...
import org.codegen.xmlio.XMLPersister
import org.codegen.xmlio.XMLDataSourcePersister
import org.codegen.util.StringHelper
import org.codegen.util.StringCategory
import org.codegen.util.JdbcUtils
import org.codegen.util.ExceptionCategory
import org.codegen.JavaModelGenerator
import org.codegen.xmlio.DataSourceReader
import org.codegen.util.OracleJDBCUtils
import org.codegen.util.OracleDataBaseMetaDataReader
import org.codegen.DataBaseMetaDataReader
import org.codegen.CodeGenerator
import org.codegen.generator.*
...
public class CodeGenMain {
  ...
  List<String> actions = []
  Set<String> availableActions = ["reverse", "write", "read", "generate", "all", "help", "datasource", "showTables"]
  ...
  public void run() {
    ...
    if (actions.contains("showTables")) {
      showTables()
    }
  ...
  public void reverseDB() {
    ...
    /* Oracle injection */
    /* Uncomment this when/if we get the schema stuff worked out. */
//    if (jdbcUtils.oracle) {
...
//    }
  ...
  private def configureJdbc() {
/* Oracle injection */
    if (jdbcUtils.oracle) {
      oracleJDBCUtils = new OracleJDBCUtils(jdbcUtils, oracleNLS)
      reader.jdbcUtils = oracleJDBCUtils
      File file = new File(this.appConfigDirFile, "oracleExcludeTables.txt")
      if (file.exists()) {
        if (debug) println "reading table exclude file ${file}"
        List<String> list = []
        file.eachLine {String line ->
          list.addAll(line.split(",") as List<String>)
        }
        if (debug) println "We are excluding these tables ${list}"
        reader.excludeTableNames = list
      } else {
        println "Unable to read Oracle Exclude Tables"
        println "To create a list of table to exclude create a file called ${file} \n" +
                " then put a comma separted list of files table names that you would like to exclude \n"
