...
//TODO just run this test, it should work with no changes.
public class Inheritance3RelationshipsTest extends TestCase {
	...
	private void deleteRoles() throws Exception {
		...
		execute(new TransactionTemplate() {
			public Object execute() {
				...
				entityManager.createQuery("delete In3Role").executeUpdate();
			...
			public Object execute() {
				...
				group.getRoles().add(
						(Role) entityManager.createNamedQuery(
								"in3.loadRole").setParameter("name",
		...
		Group loadedGroup = (Group) execute(new TransactionTemplate() {
			...
			public Object execute() {
				...
				return (Group) entityManager.createNamedQuery(
						"in3.loadGroup").setParameter("name", "sysadmins")
						.getSingleResult();
